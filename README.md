...
The `@Given` step uses a regular expression; the action class approach we use here is action-centric, not actorPage-centric, so we ignore the name of the actorPage. 
...
The Screenplay pattern describes tests in terms of actors and the tasks they perform. Tasks are represented as objects performed by an actorPage, rather than methods. This makes them more flexible and composable, at the cost of being a bit more wordy. Here is an example:
```java
    @Before
    ...
    @Given("^(.*) is on the DuckDuckGo home page")
    public void on_the_DuckDuckGo_home_page(String actorPage) {
        theActorCalled(actorPage).attemptsTo(         NavigateTo.theDuckDuckGoHomePage()     );
    }
